...
- Test stability: schema, SDK, instrumentation, UI shell, and API coverage for trace, RAG evidence, evaluation, Mimo judge mocks, golden dataset execution, golden dataset regression, persisted regression reports, run detail, API key hashing and rotation, audit logging, rate limiting, privacy redaction, retention cleanup, ingestion correctness, timeline pagination, and golden dataset contracts
...
## Cleanup Priorities
1. Plan PostgreSQL storage migration and repository contract coverage before hosted multi-user deployments.
