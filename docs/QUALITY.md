...
- Test stability: schema, SDK, instrumentation, UI shell, and API coverage for trace, RAG evidence, evaluation, Mimo judge mocks, golden dataset execution, golden dataset regression, persisted regression reports, run detail, API key security, privacy redaction, retention cleanup, ingestion correctness, timeline pagination, and golden dataset contracts
...
## Cleanup Priorities
1. Add API key hashing and rotation planning before hosted multi-user deployments.
2. Add audit logging and rate limiting before exposing the API beyond trusted local development.
3. Add larger-trace viewer tests once pagination and run summaries are exercised by real traces.
